...
def load_results(results_dir:Path, csv_filename:str) -> pd.DataFrame:
...
def run_experiment_1(exp_id, results_dir, mc_seed=None):
    ...
    results_df = load_results(results_dir, csv_filename)
    ang_spec_list = []
    counter_exp = 0
...
def run_experiment_3(exp_id, results_dir, mc_seed=None):
    ...
    base_sv_models = [['ref', 8, 13], ['alg', 8, 13]]
    additional_sv_models = list(itertools.product(['gp-steerer'], sv_nObs_choice, sv_seed_choice))
    sv_model_choices = base_sv_models + additional_sv_models
